...
export default class ChaisesPlayer {
    ...
    async rollDice(interaction: ChatInputCommandInteraction) {
        ...
        await interaction.reply({ content: successful ? `Vous vous êtes assis sur la chaise n°${result + 1}` : `Vous avez perdu toutes vos chaises!`, flags: MessageFlags.Ephemeral });
